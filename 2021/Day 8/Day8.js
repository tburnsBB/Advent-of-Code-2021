...
var fs_1 = require("fs");
var input = (0, fs_1.readFileSync)("input.txt").toString().split('\n');
...
for (var i = 0; i < input.length; i++) {
    var notes = input[i].split('|')[0].trim().split(' ');
    var display = input[i].split('|')[1].trim().split(' ');
    var one = notes.filter(function (element) { return element.length === 2; });
    var four = notes.filter(function (element) { return element.length === 4; });
    var seven = notes.filter(function (element) { return element.length === 3; });
    var eight = notes.filter(function (element) { return element.length === 7; });
    // console.log(notes)
    // console.log(display)
    // console.log(one)
    // console.log(four)
    ...
    for (var y = 0; y < display.length; y++) {
        ...
        var displayLength = display[y].length;
